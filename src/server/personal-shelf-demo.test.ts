...
describe("personal shelf catalog demo", () => {
  it("uses six exact existing records without network or synthetic nutrition", () => {
    ...
    try {
      ...
      expect(products.map((p) => assessPersonalShelfProduct(p).score)).toEqual([64, 61, null, null, 97, 54]);
  ...
  it("keeps category ranks separate and the contradictory chip outside the denominator", () => {
    ...
    expect(groups.map((g) => [g.category, g.total, g.scoredCount])).toEqual([["chips", 4, 3], ["yogurt", 2, 2]]);
