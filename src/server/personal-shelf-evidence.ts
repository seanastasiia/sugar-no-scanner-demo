...
import offSnapshot from "../../data/personal-shelf-off-evidence.generated.json";
import type { ShelfEvidence } from "@/lib/personal-shelf-rank";
import { assessPersonalShelfProduct, hasSafeShelfSource, SHELF_MODEL_VERSION } from "@/lib/personal-shelf-rank";
import { z } from "zod";
import { getSupabaseAdmin } from "./supabase";

const byId = new Map([...(snapshot as ShelfEvidence[]), ...(offSnapshot as ShelfEvidence[])].map((row) => [row.productId, row]));
...
let counts: { model: string; observations: number; complete: number; provisional: number; unscored: number } | null = null;
export function shelfEvidenceCounts() {
  if (counts) return counts;
  counts = { model: SHELF_MODEL_VERSION, observations: byId.size, complete: 0, provisional: 0, unscored: 0 };
  for (const e of byId.values()) {
    ...
    if (assessment.status === "scored") counts.complete++;
    else if (assessment.status === "provisional") counts.provisional++;
...
}).refine(hasSafeShelfSource).refine((e) => {
  const prefix = e.source === "barbora_lv" ? "barbora:" : e.source === "open_food_facts" ? "off:" : `${e.source}:`;
  return e.productId.startsWith(prefix);
});
...
export async function loadShelfEvidence(ids: string[]): Promise<Record<string, ShelfEvidence>> {
  ...
  await Promise.all(["retailer_shelf_evidence", "open_food_facts_shelf_evidence"].map(async (table) => {
    const keys = ids.filter((id) => table.startsWith("open_food_facts") ? id.startsWith("off:") : /^(?:barbora|rimi_lv|livinn_lt):/.test(id));
