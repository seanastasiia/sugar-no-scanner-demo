...
const schema = z.object({ ids: z.array(z.string().max(240).regex(/^(?:barbora:[a-z0-9-]+|(?:rimi_lv|livinn_lt):[A-Za-z0-9._~-]+|off:\d{8,14})$/)).min(1).max(MAX_SCAN_PRODUCTS) }).strict();
const limiter = createRecognitionRateLimiter();
const headers = { "cache-control": "no-store" };

export async function POST(request: Request) {
  if (process.env.PERSONAL_SHELF_RANK_ENABLED === "false") return NextResponse.json({ error: "personal_rank_disabled" }, { status: 503, headers });
