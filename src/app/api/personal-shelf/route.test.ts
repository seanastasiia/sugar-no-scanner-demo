...
describe("opt-in shelf evidence endpoint", () => {
  ...
  it("accepts exact Rimi IDs and respects the rollback switch", async () => {
    expect((await POST(request({ ids: ["rimi_lv:1000184"] }))).status).toBe(200);
    vi.stubEnv("PERSONAL_SHELF_RANK_ENABLED", "false");
    load.mockClear();
    try {
      expect((await POST(request({ ids: ["rimi_lv:1000184"] }))).status).toBe(503);
      expect(load).not.toHaveBeenCalled();
