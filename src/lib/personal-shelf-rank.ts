...
export interface ShelfEvidence {
  productId: string;
  source: "barbora_lv" | "livinn_lt" | "open_food_facts";
  sourceUrl: string;
  checkedAt: string;
  gtin: string | null;
  category: string;
...
export function hasSafeShelfSource(evidence: Pick<ShelfEvidence, "source" | "sourceUrl">): boolean {
  try {
    ...
    const hosts = { barbora_lv: ["barbora.lv", "www.barbora.lv"], livinn_lt: ["livinn.lt", "www.livinn.lt"], open_food_facts: ["world.openfoodfacts.org"] };
...
const refinedBase = /flour|starch|miltai|miltu|miltai|krakmol|ciete|мука|муки|крахмал|jahu|tarklis|rice|ryz|risi|рис|riis|protein|olbaltum|baltym|белок/;
...
export interface ShelfAssessment {
  ...
  status: "scored" | "missing_data" | "unsupported";
  score: number | null;
  missing: string[];
  components: Array<{ key: ShelfComponentKey; label: string; points: number; weight: number }>;
  reasons: string[];
  tradeoffs: string[];
  cap: string | null;
}
...
export function assessPersonalShelfProduct(product: Pick<ProductRecord, "id" | "gtin" | "category" | "format" | "shelfEvidence">): ShelfAssessment {
  ...
  const result: ShelfAssessment = { modelVersion: SHELF_MODEL_VERSION, category, status: "missing_data", score: null, missing: [], components: [], reasons: [], tradeoffs: [], cap: null };
  ...
  const fiberScore = config.balance.fiber ? Math.min(100, evidence.fiberG! / 6 * 100) : 0;
  ...
  const uncapped = Math.round(result.components.reduce((sum, component) => sum + component.points, 0));
  result.score = high.length ? Math.min(59, uncapped) : uncapped;
  ...
  result.tradeoffs = [
    ...
    ...(config.balance.fiber && evidence.fiberG! < 3 ? ["Less than 3 g fiber per 100 g"] : [])
  ];
...
export function rankPersonalShelfProducts(products: ProductRecord[]) {
  ...
  const groups = (Object.keys(SHELF_CATEGORIES) as ShelfCategory[]).flatMap((category) => {
    ...
    const scored = members.filter((entry) => entry.assessment.status === "scored")
      .sort((a, b) => b.assessment.score! - a.assessment.score! || a.product.id.localeCompare(b.product.id));
    return [{ category, label: SHELF_CATEGORIES[category].label, total: members.length, scoredCount: scored.length,
      entries: [...scored, ...members.filter((entry) => entry.assessment.status !== "scored")].map((entry) => {
        const rank = entry.assessment.score === null || scored.length < 2 ? null : scored.filter((other) => other.assessment.score! > entry.assessment.score!).length + 1;
        const tied = entry.assessment.score !== null && scored.filter((other) => other.assessment.score === entry.assessment.score).length > 1;
