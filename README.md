...
The checked-in preview contains **4,509 exact source observations: 1,287 complete scores, 2,363 provisional ranges and 859 unscored records**. The 4 September bounded expansion added 315 assessments (86 from a 200-card evidence pilot and 229 from exact-page enrichment of reviewed source categories). All 3,335 previously assessed results and both demos retain their values. These are source/SKU records, not globally deduplicated products or recognition accuracy. Original Fit and all category weights remain unchanged. Detailed scope, commands and outstanding prerequisites are in [the expansion plan](docs/personal-fit-expansion-plan.md).

Run `npm run catalog:audit:personal-fit` for an offline, read-only accounting of the entire checked-in food inventory. Add `-- --write` to save the detailed, ignored report under `.catalog-sync/`; this still makes no network, model, provider, database or application-catalog call. The current audit accounts for all **19,524 source records** exactly once (9,707 Barbora identities, 6,822 Rimi products, 2,489 Livinn identities, 6 Livin Latvia products and 500 OFF Latvia products), plus 40 curated reference rows reported separately. Of those rows, 3,335 are assessable, 3,582 belong to a supported type but still lack sufficient exact evidence, and 12,607 are outside the 19-type model. These are not globally unique products. Valid cross-source GTIN groups and conservative equal reviewed-alias/brand/pack groups are output only as review candidates; no recipe is merged or borrowed. The audit finds no usable cross-source GTIN group and 275 multilingual review candidates. It also quarantines 2,199 present but checksum/length-invalid identifiers from barcode lookup and deduplication rather than treating retailer product numbers as GTINs. Exact input hashes make later comparisons reproducible; see [whole-catalog audit](docs/test-runs/2026-09-03-personal-fit-whole-catalog-audit.md).
...
- OFF regional extraction/import commands and limits are documented in [catalog sources](docs/catalog-sources.md). Until a successful reviewed import is committed, the regional layer remains empty; prepared code is not imported data.
