...
- Welcome uses the clean shelf photo with native overlays and one 3.2-second scan-line pass. Reduced-motion mode removes it. The primary action requests camera permission only after `Open camera`. Two equal secondary actions let the visitor choose a saved photo from the gallery or open the sample shelf; neither requests camera access. A selected image enters the existing saved-photo recognition path and keeps the same no-photo-storage contract.
...
- The owner approved publishing Personal Shelf Rank to AR (Launch) on 7 September 2026. Production sets `PERSONAL_SHELF_RANK_ENTRY_ENABLED=true`, so `/` exposes the opt-in switch in `View all` and `Show demo` exposes `New rating demo`. Original Fit remains the default and is restored when the switch is off. Set only the entry flag false to hide these entry points without removing the direct demo/API; rebuild after changing it because the root page can be prerendered.
...
- The live camera fills the browser content area edge-to-edge behind the white Sugar.no wordmark and dark overlay controls. Every Sugar.no wordmark is a labelled, at-least-44 px home link: from onboarding, camera, expanded results, the demo chooser and the standalone Personal Rank demo it returns to a clean scanner root at `/`. `Show demo`, `Leave feedback` and the bottom scan status remain inside safe-area insets; the redundant “The scan starts…” caption is removed. Live video and the held frame use centered `object-fit: cover`, so screen proportions may crop the display edges. Capture still sends the complete source frame, and camera outlines use the matching cover projection; fully cropped outlines are omitted while their products remain in the results. Saved photos and demos retain `object-fit: contain` and show the complete image.
...
- Saved-photo and Checkout views omit the standalone `Photos are not saved.` footer. Reading progress, retry actions and the onboarding privacy explanation remain; photo retention behavior is unchanged. Check a saved photo while it is reading and after results appear.
...
- On the live camera, `Show demo` sits directly below the status/loading pill and matches its full available width and safe-area gutters. `Leave feedback` remains the only top-right action. The demo chooser uses the one-line `See how it works` heading, and the rating-demo entry now shares the same white 32 px row treatment as Shelf demo, Checkout demo and saved photo.
- Recovery's `Not sure — try again` uses the Sugar.no large tinted action treatment: the same 56 px height, full pill radius, white gradient rim, shadow and 17/22 px rounded label as `Show demo`, with the existing action-blue gradient instead of error red. The retry behavior and label are unchanged. Trigger an uncertain or unavailable scan, confirm the two actions have equal heights, then tap retry and confirm recognition starts again.
...
- Expanded multi-product results show the original ranked cards directly and expose the opt-in `Personal Shelf Rank · Pilot` switch. The redundant switch explanation, `Best fit first` heading, scan/basis subtitle and duplicate scan-again controls are omitted.
- Personal Shelf results keep a 24 px separation below the switch and between the compact method summary and product cards. The shared demo/recognition `How scores work` disclosure now uses the app's white gradient cards, blue signal icon, soft signal panels, compact point-weight pills, score-band legend and a separately highlighted 59-point ceiling. Its displayed thresholds are imported from the same constants used by scoring, so explanation and calculation cannot drift. Every scored card uses the same short `Why?` label followed by its existing product-specific reason; exact scores, provisional ranges, evidence and ranking logic are unchanged.
- Expanded multi-product results start with the wordmark, collapse control, optional Personal Shelf switch and ranked cards. Tapping a product opens its detail screen. Eligible Better alternatives remain available below the main content. Their horizontal carousel stays inside the content gutters and shows one full card plus about 30% of the next card, with a 12 px gap. Swipe sideways or focus the carousel and use arrow keys; cards snap near their starting edge. Long names wrap, and the last card and its retailer action can be scrolled fully into view.
...
- Uncertain recognition, provider unavailability, camera denial and offline mode have a clear recovery panel. `Not sure — try again` uses the blue tinted action for uncertain recognition and service unavailability; offline retry uses the primary black button. Both start an explicit retry without changing the automatic request limits.
...
- Nutrition resolution order is: curated catalog, exact Barbora snapshot, strict Rimi/Livin Latvia snapshot, multilingual Livinn Lithuania SKU identity and nutrition snapshot, a previously page-checked shared web card (when enabled), isolated Open Food Facts nutrition-complete bulk/API match, then exact Google Search-grounded web discovery. A separate OFF identity-only layer can resolve an exact barcode or source name earlier, but it contributes no nutrition and never creates a score by itself.
...
- A confidently identified product remains visible only while exact nutrition is being checked. When enrichment finishes without a numeric Sugar.no fit, the unresolved card is removed from compact, expanded and Personal Shelf scan results instead of displaying `Nutrition not verified`; identity-only source rows can still support a later exact lookup but do not become user-facing nutrition facts. Only source-backed protein and total sugar can produce a Sugar.no fit, and a shelf price by itself never creates a result card.
- Recognition treats a front-of-pack nutrition claim such as `17 g protein` as nutrition, never as package size. If the same detected label also contains a likely neighboring quantity, size evidence is downgraded to unknown and the existing strict brand/variant plus uniqueness-margin rules must still find one exact SKU. This lets the verified 200 g LAKTO cherry yogurt resolve without borrowing data from another flavour or pack.
...
The independent `personal-shelf-v1.6-exact-basis` model compares sugar, protein, food base and nutrient balance within **19 separate product types**. In addition to the original chips, crackers, yogurts, dairy desserts, bars, cookies and breakfast cereals, it covers popcorn and savory snacks, ice cream, bread, pasta/noodles, nuts/seeds, dried fruit, chocolate, candy, cheese, prepared meat, prepared fish and sauces/spreads. Original Sugar + Protein Fit remains the default; camera overlays, compact results, prices and Better alternatives retain their existing formula. In `View all`, enable `Personal Shelf Rank` to inspect the new comparison; disable it to return to original Fit. Missing ingredients, essential nutrients, unsupported types and contradictory tables remain unscored, not low-scored. This is a preference model, **not a validated health or safety rating**. Every category has its own versioned weights; the original seven category profiles and nutrient thresholds remain unchanged.

Personal Shelf cards pair the unchanged numeric score with a rounded gradient/rim badge: **Great fit 75–100**, **Moderate fit 50–74**, **Low fit 0–49**. The card surface itself stays neutral; fit color is confined to the labelled badge and every badge keeps at least 4.5:1 text contrast. These are presentation bands of the personal preference score, not the legacy sugar/protein Fit and not medical thresholds. A provisional range gets a colored badge only when both ends fall in one band; a crossing range (for example 71–81) gets a neutral `Moderate to Great fit` badge. Missing, contradictory and unsupported records never get a Low fit badge. Scores, ranks, evidence, camera markers and original Fit are unchanged.

Owner check: in production, upload the Turtle shelf and enable Personal Shelf Rank. The cards should be white, with 79/76 using green Great fit badges, 69/59/51/50 amber Moderate fit badges, and 40/29 red Low fit badges. Category and brand appear inside each card. Unverified Viblance cards are hidden from this ranked result. The chip demo's three rated 64/61/57–59 examples are Moderate fit; its deliberately unscored source-conflict record is filtered out before rendering. Reload to obtain the updated styles; switch Personal Shelf off to check original Fit. The expanded model was validated on `codex/personal-fit-catalog-preview` and approved for production on 7 September 2026.

The ordinary `Shelf demo` also supports Personal Shelf Rank for its four existing 55 g bars. `data/shelf-demo-evidence.generated.json` holds four separately page-checked, whole Barbora observations. A demo-only adapter checks exact legacy ID, source URL, brand, pack and source-confirmed bar form before giving the pilot a canonical SKU; it never overwrites original Fit, offers or image coordinates. ICONFIT Cookie Bliss is a cookie-flavoured bar in this view, not a cookie. Both views reuse the approved short names and local packshots. No evidence/API/provider call is made when switching the demo rating on. All four currently have a capped 59/100 and the same provisional place because saturated fat exceeds the model ceiling threshold. Fiber remains null and disclosed, not estimated; their different components remain inspectable.

Refresh only these four source records with `npm run catalog:sync:shelf-demo` (dry run) or `npm run catalog:sync:shelf-demo -- --apply`. A changed SKU, brand, pack, missing essential data, redirect or rate limit stops the refresh without replacing the previous snapshot. This demo bundle does not seed a database or expand the shared catalog. Validate with `npm run verify:catalog` and the ordinary-Shelf-demo mobile test. Owner check: Shelf demo → View all → Personal Shelf Rank must show all four under Snack bars, each 59/100; switching off must restore the original Fit and retailer actions.

Personal Shelf is now an opt-in production lane. Keep its independent preview service for future rating experiments; future model, data or presentation changes still require explicit production approval before they reach `main`.

The cereal implementation and real-photo check are recorded in [cereal shelf preview QA](docs/test-runs/2026-09-03-cereal-shelf-preview.md). The owner photo returned ten retained detections and eight source-backed Turtle ratings in that run; this is a single-photo result, not catalog-wide accuracy. The subsequent presentation-only release is recorded in [fit badge QA](docs/test-runs/2026-09-03-personal-fit-badges.md), and the 19-category catalog pass in [catalog expansion QA](docs/test-runs/2026-09-03-personal-fit-catalog-expansion.md).

The checked-in production bundle contains **7,512 exact source observations: 2,334 complete scores, 3,426 provisional ranges and 1,752 unscored records**. A scoring-only basis pass adds exact package mass/volume provenance to 178 Rimi ice-cream observations that the source reports per 100 ml. It unlocks 94 complete scores; another 84 remain honestly unscored for separate missing or contradictory fields. Raw source values remain per 100 ml and are never overwritten. The Personal Fit formula and every category weight remain unchanged. The full 7–8 September incomplete-card pass added 2,343 observations and **1,897 assessments**, taking catalog-wide assessable source rows from 3,712 to 5,609 before the basis pass. The frozen 3,605-row queue is fully accounted for: 3,600 exact-source attempts plus 5 IDs that already had exact evidence. After Barbora's recorded cooldown, the final 591-card cohort produced 544 accepted exact observations, 27 changed-SKU rejections and 20 HTTP 404 rejections; its temporary HTTP 429 was retried once after the cooldown, and no rate-limited or unattempted row remains. A subsequent owner-photo recovery added 19 whole exact-source observations and reviewed exact-SKU category mappings, bringing the audited assessable inventory to **5,760 source rows**. The independent OFF identity-only layer adds 9,626 barcode/name records but no observations or scores. These are source/SKU records, not globally deduplicated products or recognition accuracy. Barcode package labels preserve grams versus millilitres. Detailed scope, commands and outstanding prerequisites are in [the expansion plan](docs/personal-fit-expansion-plan.md) and [the ordered OFF/CSP/enrichment plan](docs/off-csp-incomplete-enrichment-plan.md).

Preview acceptance and its unresolved local WebKit startup-test limitation are recorded in [expansion QA](docs/test-runs/2026-09-04-personal-fit-expansion.md). The subsequent OFF/CSP/enrichment data, staging and live checks are recorded in [7 September catalog QA](docs/test-runs/2026-09-07-off-csp-enrichment.md). The v1.6 production release, exact live health counts and targeted Mobile Safari smoke are recorded in [8 September production QA](docs/test-runs/2026-09-08-personal-fit-v16-production.md); a physical-phone shelf check remains part of owner acceptance.

Run `npm run catalog:audit:personal-fit` for an offline, read-only accounting of the entire checked-in food inventory. Add `-- --write` to save the detailed, ignored report under `.catalog-sync/`; this still makes no network, model, provider, database or application-catalog call. The current audit accounts for all **20,120 source records** exactly once (9,707 Barbora identities, 6,822 Rimi products, 2,489 Livinn identities, 6 Livin Latvia products and 1,096 OFF records), plus 40 curated reference rows reported separately. Of those rows, 5,703 are assessable, 1,784 belong to a supported type but still lack sufficient exact evidence, and 12,633 are outside the 19-type model. These are not globally unique products. Valid cross-source GTIN groups and conservative equal reviewed-alias/brand/pack groups are output only as review candidates; no recipe is merged or borrowed. The audit finds no usable cross-source GTIN group and 275 multilingual review candidates. It also quarantines 2,199 present but checksum/length-invalid identifiers from barcode lookup and deduplication rather than treating retailer product numbers as GTINs. Exact input hashes make later comparisons reproducible; see [expansion QA](docs/test-runs/2026-09-04-personal-fit-expansion.md).

Only absent fiber can be optional in categories whose profile uses it: the UI shows a provisional range covering that category's fixed fiber contribution, always at most ten points, with both endpoints subject to the same nutrient ceilings. Values remain null; weights are not redistributed. Ranges sort by their lower bound. Any overlapping comparison has provisional places, even on a fully documented product; it is not a verified winner. A capped 59–59 interval displays `59/100` with an explicit provisional label. Yogurt, dairy dessert, ice cream, cheese, meat and fish profiles ignore fiber. A 100 ml table becomes comparable only when the exact same source title states one unambiguous package mass and volume; the scorer converts a copy with `volume ml / mass g` and retains the original observation. Per-serving, volume-only and conflicting quantity declarations remain unsupported.

Run `npm run catalog:convert:personal-fit-basis` to audit the exact dual-unit cohort and add `-- --write` to reproduce its 178 conversion records. The command fails unless the cohort is exactly 178 distinct Rimi ice creams, and records before/after statuses under `.catalog-sync/`. Run `npm run catalog:pilot:personal-fit-internet -- plan|run|report` for the frozen 100-GTIN enrichment pilot. It checks the exact OFF product first, uses grounded Google search only to discover a candidate URL, accepts no model-generated nutrient numbers, and never promotes a result. The completed pilot found 11 assessable whole-source observations, 56 exact but still incomplete observations and 81 URLs on unreviewed or unsupported hosts; no database or checked-in catalog row was written. Run `npm run catalog:simulate:personal-fit-ranges -- --write` for the read-only missing-field simulation. Of 945 newly simulated one-blocker records, 417 stay in one fit band for the full conservative interval and 528 cross a band; this report is evidence for a later policy decision, not a runtime scoring change. Full results and limitations are recorded in [8 September basis, internet and range QA](docs/test-runs/2026-09-08-personal-fit-basis-internet-ranges.md).

Source-backed composition is collected in resumable batches from existing supported-category URLs, not through per-product AI search. Rimi uses source-category breadcrumbs without the product-title slug, so a dip “for chips” is not classified as chips; its exact `sālie cepumi` leaf maps to crackers. The ingredient dictionary now includes Latvian `milti` (flour), a missing alias that had blocked many otherwise interpretable records. Exact SKU and known-GTIN mismatches are rejected, including changed retailer redirects. Original-language ingredients stay intact; the bounded EN/LV/LT/RU/ET rules do not infer nutrition from translated names. Persistent catalog-wide cross-retailer merging is not part of this release; runtime resolution can reconcile one exact observed SKU across source languages while retaining the selected source's own evidence and provenance.

Recognition treats identity-only rows as aliases, never as a successful nutrition result. It checks the confirmed local/retailer layers first, then can use an exact OFF/Livinn identity to retry the same brand, variant and observed pack against Barbora and Rimi before the bounded shared-web, OFF-nutrition and grounded-web fallbacks. A later complete result replaces every earlier identity-only result; an unrated identity is returned only after the remaining exact paths miss or stay incomplete. English `Sour Cream & Onion` and Latvian `skābais krējums un sīpoli` are reviewed equivalents for exact Pringles matching. Pack conflicts still fail closed, and a name-only identity cannot contribute an unobserved package size or GTIN. Personal Fit formula, source observations, weights and score bands are unchanged.

Two dense owner-supplied Stockmann shelf photos now exercise the same exact-source path instead of ending in an empty Personal Shelf state. Reviewed English package labels and broad-aisle exceptions are pinned to exact Rimi/Barbora SKUs for the visible Nutego, NICK'S, Pure Chocolate, The Beginnings Flapjacks, Santa Maria and Blue Dragon products. A curated card with a canonical Barbora product URL is promoted to that exact source ID before evidence lookup. Sauce composition uses the first substantive listed ingredient when water is only the carrier, while preserving the original order for the added-sugar check. Products without sufficient exact evidence remain hidden; wide-photo recognition still keeps at most ten detections and is not a complete shelf inventory. Implementation and replay evidence are recorded in [Stockmann photo QA](docs/test-runs/2026-09-08-stockmann-personal-shelf-photos.md).

Use `npm run catalog:sync:shelf-batch` for a read-only request plan. Add `-- --apply` to fetch missing observations from independent rate-limited source queues. Each source has one worker; checkpoints prevent restarting successful work. HTTP 403 or 429 stops only that source. Inspect the report before an explicit `--retry-failed`; do not retry before the source's cooling period. `SHELF_BATCH_LIMIT_PER_SOURCE=2` gives a small source-format probe. No job is scheduled automatically and no enrichment crawl runs during a scan.

Use `SHELF_BATCH_CATEGORIES=breakfast-cereal npm run catalog:sync:shelf-batch` to scope the request plan to a category; add `-- --apply` only after review. Unknown category filters fail before network requests. Older checkpoint observations cannot overwrite newer checked-in evidence. A rate-limited source stores an absolute retry boundary in the checkpoint; dry runs report the real runnable count and do not contact a cooled source.

Eight exact Turtle SKUs have reviewed English package-label aliases in `src/server/reviewed-package-aliases.ts`, pinned to their Livinn source URL, brand, pack and image. Existing Lithuanian/Latvian/Russian aliases stay intact. This identity-only overlay never copies manufacturer nutrition, completes truncated GTINs or relaxes explicit pack/flavour conflicts. A changed source/pack/image disables it. The local retailer indexes now also run in the fast recognition stage before photo-crop merging; network enrichment and online offers still wait for the background stage. This prevents known packages being discarded as unresolved names before lookup.

For an opt-in real-provider replay, run `npx tsx scripts/check-personal-shelf-photo.ts https://sugar-no-personal-rank-personal-rank-preview.up.railway.app /absolute/path/to/photo.png`. It accepts only preview/local targets, uses the client crop geometry and 1280px JPEG preparation (Sharp rather than browser canvas), then recognition, enrichment and exact product reads. Output is metadata only; photos are never copied into the repository or reports. This uses paid recognition requests; it is not part of the automatic test suite. Without `DEMO_ACCESS_CODE`, the public preview entry supplies its normal session. A replay is a single-shelf check, not an accuracy benchmark.

Retailer observations and OFF observations live in separate generated files and separate private Supabase tables. `npm run catalog:validate:shelf-pilot` reports complete/provisional/unscored counts and checks bounds against complete scores. `npm run catalog:report:personal-fit` produces an offline category calibration report with fit-band concentration, assessable coverage, range and missing-reason alerts. `/api/health` exposes the shipped evidence counts, not unique-product or visual-recognition coverage. Runtime reads at most ten exact IDs after opt-in, with a bounded local fallback; it never delays the camera, searches the internet or stores a photograph.

Try [New rating demo](https://sugar-no-personal-rank-personal-rank-preview.up.railway.app/demo/personal-shelf), also under `Show demo` when the rating entry is enabled. The standalone route now reuses the same presentational Personal Shelf renderer as a real expanded scan, with the `Best products` heading, six-signal criteria line, one collapsed `How scores work` disclosure and the same neutral cards, gradient badges, ranks, sugar/protein strip and concise `Why?` reason. Three exact rated chip examples remain: 64/61 and a real fiber-missing chip at 57–59. The contradictory unscored record is filtered out before rendering. The fixed route still mounts no scanner, requests no camera, calls no API and writes no data; scoring, evidence and supported real-scan categories are unchanged.
...
Personal Shelf scan results show only products with a numeric score or provisional score range. Missing-data, contradictory, unsupported and unresolved products are hidden; there is no `More products` section. Rated results use one `Best product`/`Best products` heading instead of visible category sections. A permanently visible compact line directly below that heading names all six scoring signals: `Sugar · Protein · Ingredients · Salt · Saturated fat · Fiber`; it clarifies the full model without returning point tables to every card. Category stays inside each card next to the brand. Each neutral card contains the score, Fit badge, meaningful within-category rank, compact sugar/protein values per 100 g and one short `Why?` line; per-card point tables and long explanations are removed. One closed page-level `How scores work` disclosure explains same-type comparison, category-dependent point weights, sugar and protein thresholds, ingredient-base treatment, salt/saturated-fat/fiber balance, the 59-point ceiling, provisional fiber ranges, score bands and the preference-score boundary. The same component renders in real recognition and the fixed demo. If the scan has no Personal Shelf ratings, one short empty state is shown without product cards. Owner check: upload the Turtle photo → View all → Personal Shelf Rank; confirm the six-signal line and lighter rated cards, open the methodology disclosure and compare its numbers to `docs/personal-shelf-rank.md`, confirm unscored cards are absent, then switch off and confirm original Fit is restored. Try a different flavour/pack and confirm it does not inherit a sibling's source.

Catalog identification and composition-rating coverage are separate. Confectionery now has chocolate and candy profiles, but a recognized package still needs exact composition evidence before it can receive a score. Not every recognized English name is a reviewed alias. A wrong/unreadable pack, different flavour, unsupported base or recipe change can still prevent assessment. Source observations are dated, not a guarantee of the recipe on a physical box; verify surprising results against its label. The scanner still retains at most ten products per scan, so a dense multi-row photo is not a complete shelf inventory.
...
### Reviewed ingredient vocabulary (v1.4)

The offline pass reviewed the 1,047 observations whose only v1.3 blocker was an unrecognized first ingredient. It unlocks 739 assessments (412 complete and 327 fiber-bounded), while 308 remain unscored. Previously assessable 2,596 results, both demos, all category weights, nutrient thresholds and original Fit are unchanged. This improves rating coverage after identification, not camera recognition accuracy.

The fallback recognizes explicit milk/cheese wording, oats/nuts/fruit, Lithuanian cocoa mass, semolina/noodles and selected animal labels. It never skips a first ingredient such as water or oil. Extracts, ambiguous compounds, malformed first-ingredient brackets and identified category conflicts stay unknown. New aliases receive the same source-zero checks as expanded categories. Raw source records, nullable nutrients and product identities are unchanged.

The historical v1.4 vocabulary audit (`npm run catalog:report:ingredient-impact -- --write`) requires its original unchanged evidence snapshot and pinned v1.3 history. After enrichment, use `npx tsx scripts/report-personal-fit-expansion.ts --write` for the current per-ID comparison against preview `b4e7453`; it fails on changed prior assessments, removed evidence or changed demo assessments. The 1,047-ID historical disposition remains in `.catalog-sync/personal-fit-alias-impact.json`.

Owner check: scan a known APP LITE apple bar or a supported yogurt/cheese package, enable Personal Shelf Rank, and check source protein/sugar against the label. Apple bars with high total sugar must still show the limiting ceiling; absent fiber remains provisional. The chip demo should look identical. Switch the pilot off to restore original Fit.

## Expansion tooling and shared Personal Fit

- The bounded 200-ID pilot is reproducible with `npx tsx scripts/personal-fit-expansion-pilot.ts plan|dry-run|run|report|promote`. Its checkpoint/candidates live under ignored `.catalog-sync/expansion-2026-09-04/`; `retry-rimi-parser` retries only the recorded parser misses, not 404s. Never reuse its frozen baseline for a different cohort.
- `npx tsx scripts/expand-personal-shelf-categories.ts plan|dry-run|run|promote` reviews 307 exact source-category IDs, writes independent candidates, and refuses promotion that changes existing evidence. v1.5 adds source-leaf aliases for nuts/seeds, long pasta, specified breads, canned fish and boxed candy; it adds no weights or product types.
- `npx tsx scripts/follow-up-off-evidence.ts plan|dry-run|run|report|promote` freezes at most 50 incomplete original OFF cards, verifies the same barcode/name/brand/pack through exact v3 responses, and preserves all other observations. It respects source cooldowns and does not repeat successful reads on resume. Unknown ingredient languages and nutrient inequalities remain unknown.
- OFF regional extraction/import added 596 accepted GTINs to the original 500-record layer, which remains unchanged. Exact commands, source version and rejection counts are documented in [catalog sources](docs/catalog-sources.md). CSV alone has no trustworthy ingredient-language metadata and adds no Personal Fit score. `follow-up-off-evidence.ts` with `--regional` checks at most 150 newly imported incomplete supported records by exact API identity (136 in this batch), with a separate checkpoint and inherited cooldowns.
- The retailer-page shared-card migrations remain available for testing, but `SHARED_WEB_CATALOG_ENABLED` and `SHARED_WEB_SHELF_EVIDENCE_ENABLED` stay false. Current Rimi, Barbora and Livinn terms do not provide a clear publication grant; Livinn explicitly requires written consent to copy or publish its information. Retailer results may help the current scan but are not automatically persisted for all users.
- `supabase/migrations/202609070001_shared_open_food_facts.sql` adds a separate server-only ODbL layer for exact dynamic Open Food Facts results. Enable `SHARED_OFF_CATALOG_ENABLED` only after that additive migration. Checksum-valid GTIN, multilingual source names, nullable composition, source date, attribution and licence stay together; no retailer data or camera image enters these tables. A composition or identity conflict blocks the record instead of merging fields.
...
This experimental retailer-page lane is disabled. Do not enable it for public shared persistence until the relevant source grants written reuse/publication permission.
...
### Shared Open Food Facts cards

- Apply `supabase/migrations/202609070001_shared_open_food_facts.sql` to the isolated target, then set the server-only `SHARED_OFF_CATALOG_ENABLED=true`. The barcode route checks bundled catalogs first, then a previously shared OFF card, then the exact OFF product API. A newly verified exact result is available to later users and server instances without another OFF request.
- The isolated `personal-rank-preview` service has this migration and flag active. Production code supports the same lane, but `SHARED_OFF_CATALOG_ENABLED` must remain false until the additive migration is applied to production. Live preview acceptance on 7 September stored two exact attributed OFF cards, one hashed name alias and two accepted immutable observations with zero conflicts. Retailer-page persistence remains governed by its separate flags and source-permission rules.
...
Production sends to `anastasiia@sugar.no` from `scanner@marketing.intend.com`, with `[Sugar.no production]` in the subject; staging is labelled separately. The Resend account quota is shared, not a reserved production quota. No DNS or existing account-key rotation is required.

Check: submit one clearly labelled QA feedback in the target environment, confirm its success state and Supabase row, then confirm the matching email in the owner's mailbox or a `Delivered` event in Resend. Check the same anonymous session's scan/feedback events in the matching Amplitude project with the correct environment property; exclude the synthetic QA session from pilot analysis. Test invalid inputs and mail failures with `npx vitest run src/server/feedback-email.test.ts src/server/amplitude.test.ts src/app/api/feedback/route.test.ts src/app/api/events/route.test.ts`. Browser tests force mail and analytics keys empty so local/E2E runs cannot send live notifications or contaminate analytics.
...
Production uses the separate EU project `Shelf Scanner Production` (`100055053`, Europe/Riga). The tab-visit UUID uses existing sessionStorage, stays stable across onboarding, new scans and reloads, and is sent as a validated optional `browserSessionId`. Individual scan IDs still rotate and remain the Supabase session/feedback keys; event metadata preserves their visit link without a schema change. Older clients without the field fall back to their scan ID. These anonymous visits are not unique people or cross-day retention. No persistent cross-day tracking, replay or autocapture is enabled. Test the funnel link with the `anonymous analytics visit` E2E scenario.

The first saved staging Amplitude chart is `Shelf Scanner Activation: Open → Scan Completed`, measuring `app_opened` to `scan_completed` within one day; it is not automatically copied into the production project. Amplitude Starter currently limits Funnel charts to two steps, so onboarding, camera-permission and feedback diagnostics remain visible as separate events rather than being added as intermediate steps to this chart. Historical events before the stable tab-visit fix may split one journey across scan identities; exclude them and labelled QA/demo traffic from launch conversion analysis.
...
1. Open [production with welcome forced](https://sugar-no-scanner-demo-production.up.railway.app/?onboarding=1) in iPhone Safari. Compare welcome typography, fit-chip rims and black buttons to Pen. Confirm `Open camera`, `Choose photo` and `Sample shelf` are all visible; choosing a photo must open `Read a saved photo` without requesting camera permission. Try a small phone and landscape.
...
6. Open `Show demo → New rating demo` when its entry is enabled, or `/demo/personal-shelf`. Confirm the page matches the expanded Personal Shelf layout: `Best products`, the six-signal criteria line, one collapsed app-style method card and three neutral chip cards at 64/61/57–59. No unscored card or per-card point table should appear. Open `How scores work`; confirm the four signal panels, category point ranges, 12% protein callout, salt/saturated-fat thresholds, 59-point ceiling, missing-data rule and score-band legend are readable without horizontal scrolling. Then tap the Sugar.no logo and confirm it returns to a clean scanner root; the direct demo must not request camera access or make an API call. Repeat the logo check from the chooser and expanded results.
7. In real results, enable `Personal Shelf Rank`. Confirm the same shorter score explanation, provisional/unknown states and continued yogurt assessment in its own category. Switch back to original Fit: its order and values must be unchanged.
...
- Barbora, Rimi and Livin Latvia can produce exact offers for their own matched SKUs. The checked-in Rimi layer contains 6,822 complete products after checking all 7,617 pages in the seven approved food and drink categories. Livin Latvia contributes 6 complete rows after checking its full 169-URL sitemap. Livinn Lithuania contributes 2,489 edible identities after accounting for all 5,926 canonical product URLs. Its runtime nutrition layer retains 1,851 conservative within-retailer identities after two duplicate records are removed without relying on invalid codes and two contradictory tables are excluded from eligible Fit counts. Lithuanian offers are not presented as Latvia purchase actions. These snapshots are not a market-wide real-time price engine.
- The checked-in Open Food Facts layer contains 1,096 records: the unchanged 500-record Latvia snapshot plus 596 accepted new regional GTINs from the streamed Latvia/Lithuania/Belarus pass. This is a bounded source snapshot, not complete market coverage. The official daily JSONL export was 12.8 GB compressed on 31 August 2026 and belongs in a scheduled data job with durable storage, not the web process or Railway build.
