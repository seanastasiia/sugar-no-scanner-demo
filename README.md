...
This cereal/retention fix is preview-only. Publish `codex/cereal-shelf-preview` to the independent `personal-rank-preview` Railway service; do not push it to `main` without production approval, because `main` also triggers production auto-deployment. Keep production integration keys out of preview.

The deployed revision, technical results and real-photo check are recorded in [cereal shelf preview QA](docs/test-runs/2026-09-03-cereal-shelf-preview.md). The owner photo returned ten retained detections and eight source-backed Turtle ratings in the verified run; this is a single-photo result, not catalog-wide accuracy.
