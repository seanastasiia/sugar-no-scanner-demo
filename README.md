...
- Latest five-product automatic web-enrichment evidence: [docs/test-runs/2026-08-25-five-product-web-enrichment.md](docs/test-runs/2026-08-25-five-product-web-enrichment.md)
...
- full-frame shelf recognition that keeps at most five high-confidence distinct readable SKUs, while repeated facings of one SKU remain grouped;
...
- a final Google Search-grounded nutrition fallback for a confidently recognized exact SKU outside the local sources; it accepts only a cited exact per-100 protein/sugar table and never estimates missing values;
...
- a vertical expanded ranking that puts the highest verified Sugar.no fit first, keeps `Great / Moderate / Low fit` attached to every rated row and places any unresolved identities last as `Nutrition not verified online`;
...
- authored Mobile Safari scenarios plus deterministic sample scenes, including progressive automatic web enrichment, landscape and long-portrait saved-image multi-pass recognition, active-camera-to-demo cancellation and a responsive matrix for iPhone 17 Pro portrait/landscape, a large iPhone and a small iPhone.

The guaranteed shelf and checkout scenes work without third-party credentials. The checkout fixture carries official manufacturer nutrition for Sproud and Schnitzer plus an explicitly labelled generic raw-chanterelle composition reference for the Stockmann pack, so all three pinned identities produce visible Sugar.no fit markers. Live camera/upload recognition names at most five readable packages, then checks the curated benchmark, the broad exact-Barbora nutrition snapshot, an exact Open Food Facts record and finally a high-confidence Google Search-grounded exact product page. Only cited per-100 protein and total-sugar values may create a fit. If every source fails, the product remains named as `Nutrition not verified online`; there is no label-scanning action and no invented rating. The app is deployed from GitHub `main` to Railway with a public HTTPS camera route. The broad prototype catalog is a versioned server-side JSON snapshot so recognition is fast and reproducible without a runtime database dependency; Supabase remains the production schema for managed catalog workflows and analytics.
...
Recognition and nutrition are separate trust levels. During the main scan Gemini may read a visible package identity, a coarse `snack / dairy dessert / other` category, a clearly printed EAN/UPC and a clearly associated physical shelf-price label. Nutrition is then hydrated from the curated benchmark, an exact product in the broad Barbora nutrition snapshot, an exact Open Food Facts barcode/product record or a final cited Google Search result for the exact SKU. The local matcher uses the supported category pack first, then requires compatible brand or visible sub-brand, rare variant words and pack or multipack size plus a clear runner-up margin. English/Latvian snack and dairy terms and common flavors are normalized; a front sub-brand such as `ProteinFit` may match an exact `BALTAIS` page only when the title, variant and pack evidence support it. When two or three exact snapshot candidates remain tied, one additional visual pass receives only those candidate IDs, titles and first-party packshots; the server accepts its choice only at confidence 0.92 or higher and rejects any slug outside the constrained set. An exact local snapshot match can produce a fit even if Barbora's live product page is temporarily unavailable; only the current online price and retailer CTA are then omitted. The grounded web fallback requires confidence of at least 0.90, an HTTPS source and exact per-100 values for the same brand/product/variant; estimates, serving conversions and neighboring flavors are rejected. If ambiguity remains, no retailer link or fit is assigned. AI never invents missing nutrition, calculates a retailer price or creates a Sugar.no fit without an exact cited record.

Live camera uses progressive resolution so network lookups do not extend the time to the first useful result. The initial response contains Gemini's identity plus any exact match already available in the checked-in curated or Barbora nutrition indexes. The frame then pauses and the product sheet becomes readable. A separate bounded, image-free request may add a current exact retailer offer, strict Open Food Facts result or cited exact web nutrition in the background. Saved-photo section passes likewise stop at local identity matching; only the final merged list of at most five products is enriched once, avoiding repeated searches for the same shelf. If enrichment fails or is superseded by `Scan again`, the initial result remains unchanged. A production benchmark found no consistent latency gain from lowering Gemini media resolution, so live frames retain the default detail-first setting until a larger accuracy/latency benchmark justifies that trade-off.

An unknown or data-poor product therefore remains named in the result, but its primary state is `Nutrition not verified online`. No label-scanning action is shown. A complete two-factor result uses the `Great fit / Moderate fit / Low fit` presentation. A product with only protein or only total sugar remains neutral and never shows a misleading approval icon or overall fit. Fiber may remain in raw source records, but it is not displayed and never affects the rating. The expanded result summary does not repeat a fit legend or marker explanation; the fit stays attached to each product marker, ranked row and badge. The price appears directly under a recognized product only when Gemini reports a separate physical price label, confidence is at least 0.90 and the exact OCR text includes a matching EUR amount. A package number, deposit or online offer cannot create it. A possible retailer candidate is never linked or displayed as a comparison. The shelf price is crossed out only when the camera price is unambiguous, the Barbora SKU match is exact and the currently fetched online price is lower. The deal card then says `Cheaper at Barbora` and offers `Buy cheaper at Barbora`; because only one retailer is connected, it never claims `best price`.

The scanner remains the primary surface after recognition. A 166 px bottom sheet exposes a best-first thumbnail preview without covering most of the shelf. `View all`, the title or the list icon expands that sheet into a full-height, internally scrollable comparison page on phones. Multiple products become a vertical `Best fit first` list: rated products are sorted from higher to lower internal fit, while products without verified nutrition stay at the end as `Nutrition not verified online` and receive no rank number. Each rated row exposes the visible fit label plus Protein and Sugar values, never the internal score. The header has one right-side collapse arrow; the title remains a second, text-only return target. Collapsing returns to the held camera frame. While the full page is open, background camera controls are removed from keyboard and screen-reader focus, Escape collapses the page and reduced-motion users receive the same state change without animation.
...
- `POST /api/recognize`: accepts a bounded image data URL or deterministic sample source. `mode: products` returns at most five package identities, optional barcodes, normalized boxes, optional camera-read shelf prices and exact retailer state. The dormant `mode: nutrition-label` contract remains for backward compatibility but has no user-facing entry point. Both modes return `imageStored: false`.
- `POST /api/resolve-products`: accepts at most five already recognized product identities and no image. It completes optional exact Barbora/Open Food Facts/cited web nutrition resolution after a camera or merged-upload result is visible and returns `imageStored: false`.
...
The sitemap includes non-food, alcohol, stale pages and pages without nutrition, so its raw count is never presented as rated-food coverage. The release reports the active-food denominator and complete automatic-fit numerator separately. EU labels usually give energy, protein and total sugar, which is why the prototype limits the fit to protein and total sugar. The grounded web fallback may transcribe these fields only from an exact cited per-100 source; it may not estimate or synthesize a missing value.
...
Google Search grounding is the last nutrition source, not a broad guessing mode. It runs only for a high-confidence visual identity after local and Open Food Facts resolution fail, requires an exact SKU, an HTTPS grounding source and per-100 values, caches successful results for 24 hours and caches misses for 30 minutes. `GEMINI_WEB_NUTRITION_MODEL` can isolate this workload from the primary vision model. Up to five unresolved identities can therefore create up to five grounded searches on a cold scan; monitor Gemini quota/cost before an open public test.
...
- [Automatic online-enrichment result](docs/screenshots/nutrition-label-fit-mobile.png)
...
The deterministic shelf fixture and pinned real-checkout result prove the multi-product interaction; they are not evidence of computer-vision accuracy. The checkout chanterelle result is a generic raw-food composition reference, not an exact Stockmann label audit. The broad Barbora index, Open Food Facts and grounded web fallback improve coverage but do not mean every package will receive an exact record or automatic rating: packaging, language, variants, barcodes, availability, community data, nutrition and prices change. External nutrition pages are not independently audited label databases. Grounded web enrichment is deliberately rejected when the exact SKU, per-100 basis, citation or confidence is ambiguous. Shelf-price OCR is shown only above its confidence threshold and still needs physical Latvian store validation. Barbora is the only connected retailer, so `best price` is not claimed. Real p95 latency, false positives, physical iPhone behavior and cross-retailer comparison remain unverified. See [Bugs.md](Bugs.md) for the live list.
