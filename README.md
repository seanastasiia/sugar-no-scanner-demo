...
The independent `personal-shelf-v1.2-bounded` preview model compares sugar, protein, food base and nutrient balance within supported product types, now including dry breakfast cereals, muesli and granola. Original Sugar + Protein Fit remains the default; camera overlays, compact results, prices and Better alternatives retain their existing formula. In `View all`, enable `Personal Shelf Rank` to inspect the new comparison; disable it to return to original Fit. Missing ingredients, essential nutrients, unsupported types and contradictory tables remain unscored, not low-scored. This is a preference model, **not a validated health or safety rating**. Cereals have their own group with sugar/protein/food-base/balance weights 30/20/25/25; no previous category weights or nutrient thresholds changed.
...
This Personal Shelf lane is preview-only. Publish `codex/personal-fit-badges-preview` to the independent `personal-rank-preview` Railway service; do not push it to `main` without production approval, because `main` also triggers production auto-deployment. Keep production integration keys out of preview.
...
The preview contains **1,533 source observations: 287 complete scores, 925 provisional ranges and 321 unscored records**, including the same 19 contradictory tables. These are source/SKU records, not globally deduplicated products or recognition accuracy. The cereal-only batch checked 291 existing URLs, adding 285 observations (98 Barbora, 125 Rimi, 60 Livinn, 2 OFF); six sources failed closed. Of these, 231 cereal records are assessable (34 complete, 197 provisional); 54 remain unscored. All 976 previously assessable records kept identical results. Recognizing explicit seed bases additionally unlocks five existing Livinn records without altering their evidence. No background continuation is running. Earlier batch history is in [source accounting](docs/test-runs/2026-09-03-personal-shelf-batch-rollout.md).

Run `npm run catalog:audit:personal-fit` for an offline, read-only accounting of the entire checked-in food inventory. Add `-- --write` to save the detailed, ignored report under `.catalog-sync/`; this still makes no network, model, provider, database or application-catalog call. The 3 September baseline accounts for all **19,524 source records** exactly once (9,707 Barbora identities, 6,822 Rimi products, 2,489 Livinn identities, 6 Livin Latvia products and 500 OFF Latvia products), plus 40 curated reference rows reported separately. It confirms 1,212 assessable records, 860 supported-type records still lacking sufficient evidence and 17,452 records outside the current seven types. These are not globally unique products. Valid cross-source GTIN groups and conservative equal reviewed-alias/brand/pack groups are output only as review candidates; no recipe is merged or borrowed. The baseline found no usable cross-source GTIN group and 275 multilingual review candidates. It also quarantines 2,199 present but checksum/length-invalid GTIN values from deduplication rather than treating source product numbers as barcodes. Exact input hashes make later comparisons reproducible; see [whole-catalog audit](docs/test-runs/2026-09-03-personal-fit-whole-catalog-audit.md).

Only absent fiber is optional in chips, crackers, bars, cookies and breakfast cereals: the UI shows a provisional range, with both endpoints subject to the original nutrient ceilings. Values remain null; weights are not redistributed. Ranges sort by their lower bound. Any overlapping comparison has provisional places, even on a fully documented product; it is not a verified winner. A capped 59–59 interval displays `59/100` with an explicit provisional label. Dairy ignores fiber. Per-serving, liquid and explicitly prepared-with-milk tables cannot become dry-cereal evidence.
...
Use `SHELF_BATCH_CATEGORIES=breakfast-cereal npm run catalog:sync:shelf-batch` to scope the request plan to cereals; add `-- --apply` only after review. Unknown category filters fail before network requests. Older checkpoint observations cannot overwrite newer checked-in evidence.
