...
## Expansion tooling and shared Personal Fit (preview, default off)
...
- The additive `supabase/migrations/202609040001_shared_web_shelf_evidence.sql` upgrades the shared-card transaction. Apply it **through Supabase to an approved isolated test database first**, then test with `SHARED_WEB_SHELF_EVIDENCE_ENABLED=true` and the existing shared-catalog flag. Default remains false. No preview database was provisioned, production credentials were not copied, and no remote migration was applied in this pass.
- This lane accepts whole Rimi/Livinn exact-page composition only after existing identity/variant/pack checks. Original language, nullable values, source URL and date are retained. Composition/core conflicts remove Personal Fit evidence and stay quarantined; later scans cannot silently repair them. Existing basic shared cards remain backwards compatible. No scan images or AI-only nutrition are promoted.
