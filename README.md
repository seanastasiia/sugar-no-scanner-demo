...
- Expanded multi-product results show the original ranked cards directly. The separate AR (My) preview can expose the opt-in `Personal Shelf Rank · Pilot` switch; launch hides it. The redundant switch explanation, `Best fit first` heading, scan/basis subtitle and duplicate scan-again controls are omitted.
...
The independent `personal-shelf-v1.6-exact-basis` preview model compares sugar, protein, food base and nutrient balance within **19 separate product types**. In addition to the original chips, crackers, yogurts, dairy desserts, bars, cookies and breakfast cereals, it covers popcorn and savory snacks, ice cream, bread, pasta/noodles, nuts/seeds, dried fruit, chocolate, candy, cheese, prepared meat, prepared fish and sauces/spreads. Original Sugar + Protein Fit remains the default; camera overlays, compact results, prices and Better alternatives retain their existing formula. In `View all`, enable `Personal Shelf Rank` to inspect the new comparison; disable it to return to original Fit. Missing ingredients, essential nutrients, unsupported types and contradictory tables remain unscored, not low-scored. This is a preference model, **not a validated health or safety rating**. Every category has its own versioned weights; the original seven category profiles and nutrient thresholds remain unchanged.
...
Owner check: in the preview, upload the Turtle shelf and enable Personal Shelf Rank. The cards should be white, with 79/76 using green Great fit badges, 69/59/51/50 amber Moderate fit badges, and 40/29 red Low fit badges. Category and brand appear inside each card. Unverified Viblance cards are hidden from this ranked result. The chip demo's 64/61/57–59 are Moderate fit; the standalone rating demo may still show its deliberately unscored source-conflict example. Reload to obtain the updated styles; switch Personal Shelf off to check original Fit. The expanded model is developed on `codex/personal-fit-catalog-preview` and may be published only to the existing isolated preview service.
...
This Personal Shelf lane is preview-only. Publish `codex/personal-fit-catalog-preview` to the independent `personal-rank-preview` Railway service; do not push it to `main` without production approval, because `main` also triggers production auto-deployment. Keep production integration keys out of preview.
...
## Expansion tooling and shared Personal Fit (preview)
...
- The isolated `personal-rank-preview` service has this migration and flag active; production does not. Live acceptance on 7 September stored two exact attributed OFF cards, one hashed name alias and two accepted immutable observations with zero conflicts. Both retailer shared flags remain false.
