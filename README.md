...
## Willingness-to-pay pilot (isolated Railway pilot)

The isolated `codex/wtp-live-pilot` experiment keeps three successful real camera/upload scans free, then offers seven days of scanner access for a one-time €2.99 payment. A compact light status badge shows the remaining free scans on live camera and uploaded-photo screens; it occupies a reserved row above camera/photo content so it cannot cover headings or media, while deterministic demos hide it. Paid access shows whole days remaining (`7 days left` through `1 day left`). An attempt counts only after at least one product has a confirmed Sugar.no rating; failed recognition, identity-only `Nutrition not verified` results and deterministic demos do not use the allowance. After verified Stripe checkout, a success screen confirms the seven-day access before the user starts the camera. When the server-verified access expires, the next real scan opens the renewal version of the paywall and the same browser can buy another independent seven-day pass for €2.99. The offer has no subscription or automatic renewal. `WTP_PAYWALL_ENABLED` defaults to `false`; AR Launch production remains disabled unless separately approved.
...
- `STRIPE_SECRET_KEY`, `STRIPE_PRICE_ID`, `STRIPE_WEBHOOK_SECRET` and environment-specific Supabase, Amplitude and Resend values stay in Railway. Ordinary staging uses Stripe test mode. The separate Railway `pilot` environment uses its own live API key and dedicated signed webhook endpoint; AR Launch production variables are unchanged.
...
Try [New rating demo](https://sugar-no-scanner-demo-production.up.railway.app/demo/personal-shelf), also under `Show demo`. Four exact chip examples use the same scorer: 64/61, a real fiber-missing chip at 57–59 and a contradictory chip without a score. The demo has only a Chips heading, without a Yogurts tab; yogurt recognition and rating remain supported in real scans. Tap a card for its component points. Both demo and recognition cards omit the long nutrient/ingredient block, source link, checked date and model footer; underlying source evidence and scoring are unchanged. Short provisional, ceiling and missing-data explanations remain. The previous isolated preview is a separate historical deployment and is not updated by this release.
