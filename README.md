...
Connected-retailer resolution runs before Open Food Facts and grounded web lookup. The Rimi matcher normalizes a small audited set of English package labels to their Latvian catalog identity while still requiring the same brand, pack size and an unambiguous top candidate. Open Food Facts matching evaluates every source-provided product-name language for the same GTIN and indexes the checked-in layer by brand, so English camera output can match a Latvian or Russian catalog name without scanning every OFF row. A language alias never bypasses brand, variant, pack-size, nutrition-completeness or ambiguity checks.
...
- The checked-in layer contains 500 complete Latvia-tagged Open Food Facts records, including 119 with at least one source-provided alternate name after the 31 August refresh. This is still a bounded proof subset, not the Latvia ceiling. The official daily JSONL export was 12.8 GB compressed on 31 August 2026 and belongs in a scheduled data job, not the web process or a Railway build.
