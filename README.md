...
Owner check: in the preview, upload the Turtle shelf and enable Personal Shelf Rank. The 79/76 cards should be green Great fit, 69/59/51/50 amber Moderate fit, and 40/29 coral Low fit. Unverified Viblance cards stay neutral. The chip demo's 64/61/57–59 are Moderate fit and its unscored chip remains neutral. Reload to obtain the updated styles; switch Personal Shelf off to check original Fit. The expanded model is developed on `codex/personal-fit-catalog-preview` and may be published only to the existing isolated preview service.
...
Personal Shelf results omit introductory boilerplate, assessed-product counters and verbose missing-data paragraphs. Supported incomplete products keep a neutral dash. Unsupported products and named packages without verified nutrition now remain as compact `More products` cards in this mode too, without invented scores or ranks. Only a genuinely empty result displays an empty-state hint. Owner check: upload the Turtle photo → View all → Personal Shelf Rank; compare cereal scores, check that unscored cards remain visible, then switch off and confirm original Fit is restored. Try a different flavour/pack and confirm it does not inherit a sibling's source.
