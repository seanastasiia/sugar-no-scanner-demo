...
Personal Shelf cards now pair the unchanged numeric score with a rounded badge and a subtle card tint: **Great fit 75–100**, **Moderate fit 50–74**, **Low fit 0–49**. These are presentation bands of the personal preference score, not the legacy sugar/protein Fit and not medical thresholds. Both the ordinary results and the compact rating demo use the same helper and scoped colors, with badge text contrast at least 4.5:1. A provisional range gets a colored badge only when both ends fall in one band; a crossing range (for example 71–81) gets a neutral `Moderate to Great fit` badge. The provisional/fiber label remains visible. Missing, contradictory and unsupported records never get a Low fit badge. Expand a card to see the band legend alongside its component points. Scores, ranks, evidence, camera markers and original Fit are unchanged.

Owner check: in the preview, upload the Turtle shelf and enable Personal Shelf Rank. The 79/76 cards should be green Great fit, 69/59/51/50 amber Moderate fit, and 40/29 coral Low fit. Unverified Viblance cards are hidden from this ranked result. The chip demo's 64/61/57–59 are Moderate fit; the standalone rating demo may still show its deliberately unscored source-conflict example. Reload to obtain the updated styles; switch Personal Shelf off to check original Fit. The expanded model is developed on `codex/personal-fit-catalog-preview` and may be published only to the existing isolated preview service.
...
Personal Shelf scan results show only products with a numeric score or provisional score range. Missing-data, contradictory, unsupported and unresolved products are hidden; there is no `More products` section. If the scan has no Personal Shelf ratings, one short empty state is shown without product cards. Owner check: upload the Turtle photo → View all → Personal Shelf Rank; compare the rated cereal cards, confirm unscored cards are absent, then switch off and confirm original Fit is restored. Try a different flavour/pack and confirm it does not inherit a sibling's source.
