...
The independent `personal-shelf-v1.4-bounded` preview model compares sugar, protein, food base and nutrient balance within **19 separate product types**. In addition to the original chips, crackers, yogurts, dairy desserts, bars, cookies and breakfast cereals, it covers popcorn and savory snacks, ice cream, bread, pasta/noodles, nuts/seeds, dried fruit, chocolate, candy, cheese, prepared meat, prepared fish and sauces/spreads. Original Sugar + Protein Fit remains the default; camera overlays, compact results, prices and Better alternatives retain their existing formula. In `View all`, enable `Personal Shelf Rank` to inspect the new comparison; disable it to return to original Fit. Missing ingredients, essential nutrients, unsupported types and contradictory tables remain unscored, not low-scored. This is a preference model, **not a validated health or safety rating**. Every category has its own versioned weights; the original seven category profiles and nutrient thresholds remain unchanged.
...
The checked-in preview contains **4,102 exact source observations: 1,225 complete scores, 2,110 provisional ranges and 767 unscored records**. The expansion collected 2,532 additional accepted pages from known Rimi and Livinn URLs; missing, redirected, contradictory and unsupported source pages failed closed. These are source/SKU records, not globally deduplicated products or recognition accuracy. Thirty observations currently fail the cross-field consistency checks. The quality report intentionally flags candy's one-band concentration, low assessable coverage for sauces and ice cream, and many bread ranges crossing a presentation band; those findings remain limitations, not hidden cleanup. No background continuation or scheduler is running. Earlier batch history is in [source accounting](docs/test-runs/2026-09-03-personal-shelf-batch-rollout.md).
...
Run `npm run catalog:report:ingredient-impact -- --write` to compare every observation and the four Shelf-demo observations against pinned v1.3 commit `47e9664`. The offline report validates unchanged input bytes, fails if an already-assessable result/demo changes, and writes the full 1,047-ID disposition to ignored `.catalog-sync/personal-fit-alias-impact.json`. It requires that baseline commit in local Git history. No external requests, uploads or scheduled jobs occur.
