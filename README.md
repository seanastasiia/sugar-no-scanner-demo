...
- Personal Shelf results keep a 24 px separation below the switch and between the compact method summary and product cards. Every scored card uses the same short `Why?` label followed by its existing product-specific reason; exact scores, provisional ranges, evidence and ranking logic are unchanged.
