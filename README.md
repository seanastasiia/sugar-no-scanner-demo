...
A [draft missing-data policy](docs/personal-shelf-missing-data-proposal.md) would add provisional score intervals when only fiber is absent, without redistributing its weight or inventing grams. The read-only audit found 82 such records in addition to the 64 complete scores. This proposal is **not active**; essential-field and contradiction gates remain unchanged. It includes a rounding edge that must be addressed before implementation.
...
To try it immediately, open the [New rating demo](https://sugar-no-personal-rank-personal-rank-preview.up.railway.app/demo/personal-shelf), or choose `Show demo → New rating demo`. No camera, photo or provider call is needed. Its compact Shelf-photo-style list shows `Best fit first`, a small `Demo` badge and a `Chips / Yogurts` category selector. Each row contains the local rank, packshot, brand/title, neutral score and protein/sugar per 100 g. Introductory sections, count prose, `Consider`, separate disclosure buttons and the footer are removed. Tap a whole card to expand component points, original ingredients and its dated exact source; tap again to collapse. The top-right chevron returns to the scanner.

The five selected real catalog products retain the exact same sources and model as scan results: two scored chips (64/61), a chip with contradictory source data (`Not scored`, no nutrition claim), and two spoonable yogurts (97/54). Switching categories never combines their ranks; the unscored chip has a dash rather than a numeric place. This is a labelled catalog example, not simulated recognition or a representative market sample. Packshot failures show a neutral placeholder without hiding evidence. Owner check: switch Chips → Yogurts → Chips, open/close a card, confirm protein and sugar remain readable, then return to the scanner. The compact renderer is demo-only; normal scan results and their existing `Why this score?` disclosures are unchanged.
