...
- The compact camera preview scrolls vertically in a single column, with one complete card and about 30% of the next to invite scrolling. Its height follows the first card when names wrap or the viewport changes; a single result needs no empty peek area. Native scrolling, gentle vertical snapping and keyboard arrows keep every card reachable while `Scan again` and `View all` stay below the scroll area. Names use 15/20 px type, nutrition 13/18 px, and cards retain rank, fit, source-backed sugar and optional Carbs. Short landscape screens keep the existing action-only compact fallback; `View all` opens the full list. To check: open a demo, swipe the compact cards up to the last product, open it, return and verify both bottom buttons stay accessible.
