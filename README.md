...
Try [New rating demo](https://sugar-no-personal-rank-personal-rank-preview.up.railway.app/demo/personal-shelf), also under `Show demo` when the rating entry is enabled. Four exact chip examples use the same scorer: 64/61, a real fiber-missing chip at 57–59 and a contradictory chip without a score. Cards follow the `Best fit first` header directly, without a Chips capsule or Yogurts tab; the chip section keeps its accessible name, and yogurt recognition and rating remain supported in real scans. Tap a card for its component points. Both demo and recognition cards omit the long nutrient/ingredient block, source link, checked date and model footer; underlying source evidence and scoring are unchanged. Short provisional, ceiling and missing-data explanations remain. The independent Personal Shelf preview remains a separate deployment.
...
6. Open `Show demo → New rating demo` when its entry is enabled, or `/demo/personal-shelf`. Confirm four chip cards directly below the header with no Chips capsule: scores 64/61, a provisional 57–59 chip and one unscored chip. Tap a card: four component-point rows remain, without full nutrition, original ingredients, source links, checked dates or a model footer. Short uncertainty explanations remain; the direct demo must not request camera access.
