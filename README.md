...
The independent `personal-shelf-v1.5-bounded` model compares sugar, protein, food base and nutrient balance within **19 separate product types**. In addition to the original chips, crackers, yogurts, dairy desserts, bars, cookies and breakfast cereals, it covers popcorn and savory snacks, ice cream, bread, pasta/noodles, nuts/seeds, dried fruit, chocolate, candy, cheese, prepared meat, prepared fish and sauces/spreads. Original Sugar + Protein Fit remains the default; camera overlays, compact results, prices and Better alternatives retain their existing formula. In `View all`, enable `Personal Shelf Rank` to inspect the new comparison; disable it to return to original Fit. Missing ingredients, essential nutrients, unsupported types and contradictory tables remain unscored, not low-scored. This is a preference model, **not a validated health or safety rating**. Every category has its own versioned weights; the original seven category profiles and nutrient thresholds remain unchanged.
...
The checked-in preview contains **6,949 exact source observations: 2,078 complete scores, 3,078 provisional ranges and 1,793 unscored records**. The 7 September incomplete-card pass added 1,799 observations and **1,444 assessments**, taking catalog-wide assessable source rows from 3,712 to 5,156 without changing any of the 5,150 previous observations' rating outcomes. The frozen 3,605-row queue was fully attempted for Rimi, Livinn and OFF; Barbora stopped on the provider's 429 boundary after 2,053 of 2,644 queued rows, leaving 591 unattempted until the source cooldown expires. The independent OFF identity-only layer adds 9,626 barcode/name records but no observations or scores. These are source/SKU records, not globally deduplicated products or recognition accuracy. Original Fit's formula and all category weights remain unchanged. Barcode package labels preserve grams versus millilitres. Detailed scope, commands and outstanding prerequisites are in [the expansion plan](docs/personal-fit-expansion-plan.md) and [the ordered OFF/CSP/enrichment plan](docs/off-csp-incomplete-enrichment-plan.md).
