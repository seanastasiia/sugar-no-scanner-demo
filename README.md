...
Technical validation uses `npm run verify` and the Mobile Safari suite (`CI=1 E2E_PRODUCTION=1 E2E_PORT=3012 npm run test:e2e`). `E2E_PORT` defaults to 3000 and lets an isolated checkout avoid another project's running server. Screenshots and validation logs are stored under ignored `test-results/`; browser tests mock external failures and feedback writes.
