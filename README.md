...
- A crossed-out shelf price and `Buy cheaper at Barbora` appear inside that product's compact or horizontal ranked card only when the exact Barbora SKU is currently cheaper. The expanded view has no separate price-comparison block. Similar options load their exact current Barbora prices after the scan result and expose their own `Buy online` actions; there is no shared retailer button.
...
2. Scan a shelf with more than ten visible products and confirm no more than ten distinct results appear; repeated facings still count once.
...
6. Confirm a physical price appears only when a price label is visible and an exact cheaper Barbora result is clearly qualified inside the matching product card, with no separate price-comparison block below the ranking.
...
- Grounded web nutrition has variable latency and cost. A dense scan can require up to ten bounded exact-product lookups; production should persist human-reviewed successful results in Supabase.
