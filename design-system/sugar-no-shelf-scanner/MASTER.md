...
| 04 `BmBpL` Compact | Horizontal ranked-card carousel, one full card + about 30% next; fixed Scan again / View all below |
...
The welcome distributes header, introduction, preview and actions with 24 px minimum gaps; short screens reduce the photo height. Compact packshots are 56 px, and the detail packshot is square up to 290 px. Compact cards use a 96 px minimum height, 12 px padding/gap, 15/20 px names and 13/18 px nutrition labelled `Sugar 2.3 g`; the horizontal card width is (available width − 8 px gap) / 1.3, revealing one full card and 30% of the next on the right. Equal row heights adapt when content wraps. The carousel includes all displayed products and uses native horizontal proximity snapping; a single card fills the available width. The list includes verified protein and separates informational prices from the fit row. Feedback uses 8 px internal gaps, a fixed header/footer and scrollable body sized to the visual viewport; the success mark is green with a white check.
