# Personal Shelf Rank v1.6 — exact-basis preview contract

Status: v1.6 adds exact package-mass/volume conversion for a bounded per-100-ml cohort in the nineteen-type isolated preview, 8 September 2026; weights and nutrient curves are unchanged. It is not clinically validated and does not replace default Fit. Model ID: `personal-shelf-v1.6-exact-basis`. Production keeps its separately approved release. [Preview](https://sugar-no-personal-rank-personal-rank-preview.up.railway.app/).
...
Current checked-in coverage is 7,493 exact source observations: 2,332 complete scores, 3,371 provisional ranges and 1,790 unscored observations. Across the full 20,120-row source inventory, 5,703 rows are assessable, 1,784 are in supported types but lack sufficient exact evidence, and 12,633 are outside the model. These are source rows, not globally deduplicated physical products or a recognition benchmark. The offline quality report flags candy's concentration in Low fit and many bread ranges that cross a presentation band. Never borrow another product's nutrients to improve coverage. Curated/demo/grounded-web identities without a separate compatible observation stay unscored in this pilot, even when they have original Fit.
