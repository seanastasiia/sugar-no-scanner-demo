...
6. `src/server/demo-scenes.ts` supplies the deterministic Shelf and Checkout fixtures. They prove the UX, not real-world CV accuracy. Only when Personal Shelf is opened on the Shelf fixture, `src/lib/shelf-demo-personal-rank.ts` maps the four exact curated identities to whole canonical observations from `data/shelf-demo-evidence.generated.json`. Source-confirmed bar form overrides the legacy ICONFIT cookie category only in the pilot copy; thumbnails map to the unchanged scene IDs. `scripts/sync-shelf-demo-evidence.ts` refreshes these four pages atomically, never the full catalog. No demo-toggle evidence/provider request is needed.
...
12. `/demo/personal-shelf` is a separate camera-free catalog example, reached directly or through the demo chooser. `src/server/personal-shelf-demo.ts` selects four exact existing chip records without I/O, including a real missing-fiber 57–59 example; only those records are passed into the client `PersonalShelfDemo`. Its compact Shelf-photo-style renderer calls the same pure model, places cards directly below the header without a category capsule, retains the accessible Chips section name and expands component points from a whole-card button. Both demo and scan disclosures omit the full evidence/source/model text, without changing stored evidence or assessment rules; concise uncertainty/ceiling explanations remain. Real scans still support every pilot category, including yogurts. The demo does not mount the scanner or background evidence refresh. It has no provider calls or mock recognition frames. The original Shelf/Checkout scene contracts remain unchanged.
13. The v1.2 preview adds a dry breakfast cereal/muesli/granola group and keeps unsupported/unresolved names visible in `PersonalShelfResults`. `reviewed-package-aliases.ts` contains identity-only, source-pinned labels for eight Turtle packages. `external-catalog.ts` rejects additional flavour tokens for these reviewed labels; its in-memory indexes are consulted in fast recognition before crop merging, while offers and network enrichment remain deferred. `check-personal-shelf-photo.ts` is an opt-in, preview/local-only real-provider replay with metadata-only output. `SHELF_BATCH_CATEGORIES` scopes the reproducible evidence fetch; newer observations win over older checkpoints.
14. `src/lib/personal-shelf-fit.ts` maps the existing personal score to presentation bands (75+/50–74/0–49), without changing numeric scoring or original Fit. Scan results and the compact demo share `personal-shelf-fit-badge.tsx` and its scoped badge/card colors. Cross-band provisional ranges use neutral dual-band labels; unknown assessments get no badge.
15. The willingness-to-pay lane is feature-flagged by `WTP_PAYWALL_ENABLED`. `src/lib/wtp-access.ts` owns the local three-scan allowance and bounded UTM capture; `src/components/paywall-dialog.tsx` owns the accessible offer/restore UI. `/api/billing/*` creates Stripe Checkout, verifies payment, processes the signed webhook and restores a paid entitlement. `src/server/billing.ts` hashes raw browser tokens before Supabase and never exposes Stripe secrets to the client.
