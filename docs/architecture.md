...
12. `/demo/personal-shelf` is a separate camera-free catalog example, reached directly or through the demo chooser. `src/server/personal-shelf-demo.ts` selects four exact existing chip records without I/O, including a real missing-fiber 57–59 example; only those records are passed into the client `PersonalShelfDemo`. Its compact Shelf-photo-style renderer calls the same pure model, uses a non-interactive Chips heading and expands component points from a whole-card button. Both demo and scan disclosures omit the full evidence/source/model text, without changing stored evidence or assessment rules; concise uncertainty/ceiling explanations remain. Real scans still support every pilot category, including yogurts. The demo does not mount the scanner or background evidence refresh. It has no provider calls or mock recognition frames. The original Shelf/Checkout scene contracts remain unchanged.
13. The isolated willingness-to-pay lane is feature-flagged by `WTP_PAYWALL_ENABLED`. `src/lib/wtp-access.ts` owns the local three-scan allowance and bounded UTM capture; `src/components/paywall-dialog.tsx` owns the accessible offer/restore UI. `/api/billing/*` creates Stripe Checkout, verifies payment, processes the signed webhook and restores a paid entitlement. `src/server/billing.ts` hashes raw browser tokens before Supabase and never exposes Stripe secrets to the client. Production remains off until an explicit release.
