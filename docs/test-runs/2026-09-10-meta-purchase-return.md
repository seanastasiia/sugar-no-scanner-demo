...
New Stripe sessions return through `/api/billing/return`. This header-only 303 response has an explicit no-referrer policy, strips all input except a bounded session ID, remains on the same origin and renders no third-party code. Legacy direct successful returns with a detailed referrer use this boundary before verification. Access still requires the existing server-side Stripe and token-ownership validation. No extra payment, refund, schema change or CAPI integration is part of this fix.
