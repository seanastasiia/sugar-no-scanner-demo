...
Owner acceptance: confirm the email on the phone, check “Owner access”, scan and reload; public `/` remains metered. Install the Personal Shelf URL again to use its own manifest. An isolated iOS Home Screen session may require another email confirmation. Links remain usable for their 15-minute validity; sessions last one year and can be revoked by rotating the server secret. Recognition rate limits and research quotas remain.

The initial published implementation `6b1f1af68e8d99a713f39272b31bd73fdaa41560` passed live WebKit and Chromium: owner session persisted, public paywall remained, zero owner funnel calls or browser errors, Secure/HttpOnly/path-scoped cookie and pilot manifest verified. A final copy-only refinement shortens the badge to “Owner access”; its two Mobile Safari isolation tests passed again (6.1 seconds).
