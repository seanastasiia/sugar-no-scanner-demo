...
The compact sheet keeps a short best-first preview. Expanding it turns the recognized set into a vertical `Best fit first` list rather than another horizontal carousel. Each rated row carries its rank, plain-language fit and the two verified inputs. If exact nutrition cannot be confirmed, a confidently named package remains as a neutral unrated row instead of vanishing; anonymous price-only findings and generic placeholders are not shown. The complete scan is capped at ten high-confidence distinct SKUs while lookup concurrency stays bounded.
