...
The compact sheet keeps a short best-first preview. Expanding it turns the recognized set into a vertical `Best fit first` list rather than another horizontal carousel. Each rated row carries its rank, plain-language fit and the two verified inputs. Products without a complete fit remain useful but sit after the ranking as `Nutrition not verified online`, without a number, inferred nutrition or label-scan action. The complete scan is capped at five high-confidence distinct SKUs so the result stays readable and automatic enrichment remains bounded.
