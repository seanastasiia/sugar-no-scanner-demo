...
5. [ ] **Activate the scan-to-catalog composition loop.** Source review permits shared reuse of Open Food Facts under ODbL, but did not establish permission to republish retailer-page data; Livinn explicitly forbids copying/publishing without written consent. The implementation therefore persists only exact OFF results in a separate attributed layer. Its additive migration, conflict quarantine, server-only access and fresh-instance reuse pass isolated tests. Applying the migration and flag to the isolated staging Supabase target still requires action-time owner confirmation. Retailer shared flags remain off. A photo-label contribution flow remains a separate opt-in UI change, not automatic retention of scans.
6. [x] **Verify and release preview.** Updated README/Bugs, pushed only the preview branch and verified Railway SUCCESS plus exact HTTPS health, data and Mobile Safari-emulated browser smoke. Final implementation passes 643 tests and the build. The full local mobile run passed 57/58 with one unresolved initial-navigation failure; this is a qualified preview acceptance, not a clean final 58/58 run. Step 5 remains pending.
...
- Permission review on 7 September narrowed live shared persistence to Open Food Facts. The OFF-only migration creates separately attributed ODbL product and immutable-observation tables; exact barcode/API records are normalized to checksum-valid GTINs, bounded, and blocked permanently when later identity or composition conflicts. Retailer-page results remain scan-local because no reusable publication grant was established. Local SQL/server/API tests pass; staging activation is pending the required action-time confirmation.
- Candidate `245cf8927c1b94ae3eaef5f79e843fbb9614eab0` passes 662 unit/integration tests, every catalog/Personal Shelf validator, the production build and 58/58 Mobile Safari scenarios. Exact normalized name aliases are reusable without storing raw scan text; a competing GTIN blocks the alias. Detailed evidence: [7 September shared OFF acceptance](test-runs/2026-09-07-shared-off-preview.md).
