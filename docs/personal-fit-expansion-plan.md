...
5. [x] **Activate the scan-to-catalog composition loop.** After explicit owner approval, the additive OFF-only migration was applied to the isolated staging Supabase project and only `SHARED_OFF_CATALOG_ENABLED` was enabled on the preview. Exact barcode and exact name results now persist as separate attributed ODbL cards; one-way name aliases are shared without raw scan text. RLS, service-only promotion, immutable observations, idempotence and conflict quarantine were verified live. Retailer shared flags remain off because no publication permission was established. A photo-label contribution flow remains a separate opt-in UI change, not automatic retention of scans.
6. [x] **Verify and release preview.** README/Bugs and the acceptance record are current; only the preview branch and isolated Railway service were changed. The final implementation passes 663 unit/integration tests, all catalog validators, the production build and 58/58 Mobile Safari scenarios. Railway, exact HTTPS health, real OFF barcode/name resolution, inline Personal Fit cards, database persistence and repeat reuse all pass. Production remains unchanged.
...
- Shared-card composition code and the additive SQL migration pass isolated PostgreSQL tests, including language/basis/GTIN agreement, permanent conflict quarantine, idempotence and browser-role denial. The migration is now active only in isolated staging. Live verification confirms RLS browser denial, service-only read/promotion and immutable history. No production database or image retention was introduced.
- Permission review on 7 September narrowed shared persistence to Open Food Facts. Exact barcode/API records are normalized to checksum-valid GTINs, bounded and blocked permanently when later identity or composition conflicts. Retailer-page results remain scan-local because no reusable publication grant was established.
- Implementation `df98ff16ec7621bb1a3af50386a743cae2c77226` passes 663 unit/integration tests, every catalog/Personal Shelf validator, the production build and 58/58 Mobile Safari scenarios. Exact normalized name aliases are reusable without storing raw scan text; a competing GTIN blocks the alias. Live staging contains two accepted OFF cards, one hashed name alias, two immutable observations and zero conflicts. Detailed evidence: [7 September shared OFF acceptance](test-runs/2026-09-07-shared-off-preview.md).
...
- Final candidate `380ef66131232428d2beaf396b9f68ea7d37cba5`, containing implementation `823f83c`, reached Railway SUCCESS as deployment `8937cee1-cbcf-44ed-905b-62983c2f69a0`. HTTPS acceptance on 4 September at 14:30 UTC passed: exact revision/counts, four new source assessments, regional barcode grams/millilitres, ordinary Shelf/Checkout and the chip demo. The saved mobile-emulation screenshot was visually inspected. Main and live production remain `ab813c710f41ee7423f19ff35acb95381140c98d`. Local WebKit startup instability remains documented in the QA log; no source data, test assertions or security controls were relaxed.
