...
| Rating legend reflects data | Legend renders only when at least one product has complete verified nutrition; generic results say `Identified, not rated` | Pass locally |
...
| Exact Barbora link and current price | Live page parser, brand guard, candidate-margin tests and UI end-to-end assertions allow only an exact SKU to drive the link or crossed-out shelf price | Pass locally |
