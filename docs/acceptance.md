...
| Camera focuses on Sugar.no ratings | Boxes and colored markers render only for products with complete verified nutrition; generic identities stay in the result sheet without an `i` overlay | Pass locally |
| Rating legend reflects data | Legend renders only when at least one product has complete verified nutrition; an all-unrated scan explicitly says that identified products were not highlighted | Pass locally |
