...
| Checkout demo | One real checkout-belt photo returns the three packaged identities Gemini could read (Sproud, Schnitzer and Stockmann), three fit-coloured camera overlays and a three-row best-first list. Sproud and Schnitzer use official manufacturer nutrition; chanterelles use an explicitly labelled generic raw-food composition reference | Pass locally and in production as an interaction fixture, not a CV benchmark |
...
| Railway HTTPS and live health check | Railway deployment `5e549118-f593-4677-af8a-ac99e23f7069` succeeded for commit `6720faca2ec5f8f60cbd61ab45e39dccc4d28658`; `/api/health` reported the same commit. Production API and Mobile Safari smoke returned three complete checkout fits, three labelled markers and no horizontal overflow | Pass in production |
