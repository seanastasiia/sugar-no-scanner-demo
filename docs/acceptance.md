...
| On-demand Barbora nutrition | An exact food SKU is fetched after recognition; unit tests require source-backed energy, protein and total sugar for the two-factor fit. Protein-only, sugar-only and identity-only states remain neutral | Unit pass; browser execution pending outside managed sandbox |
...
| Multi-product fit order | The expanded dialog renders a vertical best-first list. Complete Protein/Sugar fits sort from higher to lower score with visible fit text and values; identity-only or incomplete items remain stable at the end as `Fit pending` without a rank | Unit pass; browser regression written |
...
| Camera focuses on Sugar.no ratings | Complete protein-plus-sugar fits use `Great / Moderate / Low` markers. One-factor and identity-only detections remain available in the result sheet but never receive a camera marker, overall fit or misleading approval icon | Unit pass; browser execution pending outside managed sandbox |
