...
| Multi-product fit order | The expanded dialog renders a vertical best-first list. Complete Protein/Sugar fits sort from higher to lower score with visible fit text and values; identity-only or incomplete items remain stable at the end as `Fit pending` without a rank. Public smoke verified both the four-item ranked shelf and three-item pending checkout states | Pass in production |
