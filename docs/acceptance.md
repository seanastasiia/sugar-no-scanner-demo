...
| On-demand Barbora nutrition | An exact food SKU is fetched after recognition; unit tests and Mobile Safari prove a sourced `2 of 3` quick view while pages without nutrition remain unrated | Pass locally |
...
| Several products from one live shelf frame | Broad recognition scans the complete frame and returns up to eight distinct readable front-facing SKUs; unit coverage verifies the prompt contract, Mobile Safari renders three different rated products in one held result and a production `gemini-3.7-flash` call returned eight detections from the shelf concept image in 5,012 ms | Pass on concept image |
...
| Camera focuses on Sugar.no ratings | Boxes and colored markers render only for products with a numeric full badge or quick view; generic identities stay in the result sheet without an `i` overlay | Pass locally |
