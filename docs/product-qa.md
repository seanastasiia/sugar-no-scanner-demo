...
9. Return to live camera, tap `Show demo`, then `Checkout demo`. Confirm it shows the real checkout-belt photo and returns three named packaged products at once: Sproud, Schnitzer and Stockmann. Because nutrition is not verified for these identities, the expanded list must say `Fit order pending`, place all three after the rated ranking boundary and show no camera fit markers or invented Sugar.no result.
...
9. Put several identical facings of one SKU in the frame. Confirm the status and ranked list count them as one unique product, not one product per package.
...
19. Keep an unmatched shelf steady for at least one minute. Confirm the app does not fall into repeated generic errors. If the server responds with a rate limit, scanning must stop, honor the shown retry delay, preserve any confirmed product and keep retry/demo actions available.
