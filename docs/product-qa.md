...
8. If the first broad pass is uncertain, keep the shelf steady. Confirm the app stays honest, does not guess a centered product and automatically submits a later usable scene without a shutter action.
...
11. After a successful live scan, move the phone slightly. Confirm the video remains live and boxes follow the packages. Then close the fridge or point at an unrelated scene; stale boxes and cards must clear after two tracking samples and the new scene must be scanned automatically.
