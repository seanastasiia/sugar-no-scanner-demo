...
7. Swipe the Similar options cards left and right. Confirm that they are comparable formats, remain usable without horizontal page overflow and each exact Barbora option has its own `Buy online` action with the current price. There must be no shared retailer button below the carousel.
...
7a. Scan a high-confidence exact product absent from local Barbora/Open Food Facts data. Confirm the held identity appears first, then remains only if an exact cited per-100 web record exists. There must be no `Scan nutrition label` button. A source miss, serving-only page or neighboring flavor must disappear after lookup.
...
9. Point at a shelf containing more than ten different front-facing readable products. Confirm one broad pass returns several distinct products at once rather than only the center package, but never more than ten distinct SKUs. Repeated facings of one SKU must count once. If the broad pass first finds one product and the completion pass finds nothing else, the confirmed product must remain visible and locked for reading.
...
13. Include the product and one clearly associated shelf label in the same frame. Confirm the camera price appears directly beneath the recognized product in the compact sheet and ranked row. If an exact Barbora match is cheaper, confirm both views show a one-tap `Buy cheaper at Barbora` action with the online price inside the matching product card. It must open the exact SKU directly. Confirm there is no separate price-comparison panel below the ranked list.
...
15. Confirm `Great fit / Moderate fit / Low fit` markers appear only for products with a numeric full badge. A visual-only or price-only result should have no camera marker and must disappear after lookup without exposing its shelf price, retailer action or a label-scan action.
...
- When an automatic match has no exact cited nutrition, does it disappear after lookup without leaving a price-only or identity-only card?
