...
7. Show two visually similar same-brand variants or pack sizes. Confirm the correct exact SKU is selected only when its visible packaging distinguishes it; an unreadable size must remain `Nutrition not verified online` instead of borrowing the neighboring candidate's fit.
7a. Scan a high-confidence exact product absent from local Barbora/Open Food Facts data. Confirm the held identity appears first, then the result is automatically enriched only if an exact cited per-100 web record exists. There must be no `Scan nutrition label` button. A source miss, serving-only page or neighboring flavor must stay neutral.
...
15. Confirm `Great fit / Moderate fit / Low fit` markers appear only for products with a numeric full badge or quick view. A visual-only Coca-Cola or Activia result should have no camera marker; the sheet must say `Nutrition not verified online` and must not offer a label-scan action.
...
- When an automatic match has no exact cited nutrition, is `Nutrition not verified online` honest and visually secondary rather than making the whole scan look broken?
