...
6. Point at a clear food outside the 40 curated snacks. Keep a visible EAN/UPC in frame when practical. Confirm the app first tries the exact Barbora and Open Food Facts records. If either exact source supplies energy, protein and total sugars, a `Great fit`, `Moderate fit` or `Low fit` marker and a two-factor Sugar.no badge should appear. Fiber must not appear or affect the result.
