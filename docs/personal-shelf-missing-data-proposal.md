# Missing-data policy proposal

Status: draft, 3 September 2026. Not implemented or deployed. The active model remains `personal-shelf-v1.0-pilot`, with 64 complete scores. This document responds to the owner's request to avoid blocking a useful result when an optional field is absent. The exact partial-score presentation and ranking policy need owner acceptance before activation.
...
- One rounding edge must be fixed before implementation: synthetic fiber endpoints for `livinn_lt:1AM070101204` gave 14 and 25 despite a 10-point component difference. Binary floating-point summation made 6.1 + 6.3 + 2.1 slightly less than 14.5. Sum integer tenths before final rounding and cover the correction with a model-version regression test. No source record or active score was changed during this analysis.

## Acceptance before activation

Technical: preserve exact-identity/source/contradiction checks; prove null fiber remains null; ensure bounds contain the corresponding complete score; removing known fiber must not raise the lower bound; preserve score ceilings at both endpoints; test zero versus absent versus invalid input, rounded boundaries, ties and overlapping intervals. Audit complete-score changes caused by the rounding fix rather than silently claiming exact parity. Run full scoring and browser verification only when implementation is requested.

Owner: compare one complete product with one fiber-missing product; confirm the interval and provisional place are understandable without opening a long explanation; confirm a salt-missing or contradictory product does not look fully rated. Only then activate this policy in the isolated preview. Production remains a separate gate.
